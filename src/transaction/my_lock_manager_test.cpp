...
TEST_F(LockManagerTest, TransactionStateTest) {
    ...
    std::thread t0([&] {
        printf("t0\n");
        Transaction txn0(0);
        bool res = lock_manager_->lock_shared_on_record(&txn0, rid, tab_fd);
        EXPECT_EQ(res, true);
    ...
    std::thread t1([&] {
        ...
        Transaction txn1(0);
        bool res = lock_manager_->lock_shared_on_record(&txn1, rid, tab_fd);
        EXPECT_EQ(res, true);
...
TEST_F(LockManagerTest, BasicTest1_SHARED_TUPLE) {
    ...
    for(int i = 0; i < num; ++i) {
        ...
        EXPECT_EQ(i, txns[i]->get_transaction_id());
    }

    auto task = [&](int txn_id) {
        bool res;
        for(const Rid &rid : rids) {
            res = lock_manager_->lock_shared_on_record(txns[txn_id], rid, txn_id);
            EXPECT_TRUE(res);
        ...
        EXPECT_EQ(txns[txn_id]->get_state(), TransactionState::COMMITTED);
    };
...
TEST_F(LockManagerTest, BasicTest2_EXCLUSIVE_TUPLE) {
    ...
    auto task = [&](int txn_id) {
        bool res;
        for(const Rid &rid : rids) {
            res = lock_manager_->lock_exclusive_on_record(txns[txn_id], rid, txn_id);
            EXPECT_TRUE(res);
            EXPECT_EQ(txns[txn_id]->get_state(), TransactionState::GROWING);
        }
...
TEST_F(LockManagerTest, BasicTest3_SHARED_TABLE) {
    ...
    auto task = [&](int txn_id) {
        ...
        for(const int tab_fd : tab_fds) {
            res = lock_manager_->lock_shared_on_table(txns[txn_id], tab_fd);
            EXPECT_TRUE(res);
            EXPECT_EQ(txns[txn_id]->get_state(), TransactionState::GROWING);
        }
...
TEST_F(LockManagerTest, BasicTest4_EXCLUSIVE_TABLE) {
    ...
    auto task = [&](int txn_id) {
        bool res;
        for(const int tab_fd : tab_fds) {
            res = lock_manager_->lock_exclusive_on_table(txns[txn_id], tab_fd);
            EXPECT_TRUE(res);
            EXPECT_EQ(txns[txn_id]->get_state(), TransactionState::GROWING);
        }
...
TEST_F(LockManagerTest, BasicTest5_INTENTION_SHARED) {
    ...
    std::thread t0([&] {
        Transaction txn0(0);
        bool res = lock_manager_->lock_IS_on_table(&txn0, tab_fd);
        EXPECT_EQ(res, true);
        EXPECT_EQ(txn0.get_state(), TransactionState::GROWING);
        res = lock_manager_->lock_shared_on_record(&txn0, rid, tab_fd);
        EXPECT_EQ(res, true);
        EXPECT_EQ(txn0.get_state(), TransactionState::GROWING);
    ...
    std::thread t1([&] {
        ...
        Transaction txn1(1);
        bool res = lock_manager_->lock_exclusive_on_table(&txn1, tab_fd);
...
TEST_F(LockManagerTest, BasicTest6_INTENTION_EXCLUSIVE) {
    ...
    std::thread t0([&] {
        Transaction txn0(0);
        bool res = lock_manager_->lock_IX_on_table(&txn0, tab_fd);
        EXPECT_EQ(res, true);
        EXPECT_EQ(txn0.get_state(), TransactionState::GROWING);
        res = lock_manager_->lock_exclusive_on_record(&txn0, rid, tab_fd);
        EXPECT_EQ(res, true);
        EXPECT_EQ(txn0.get_state(), TransactionState::GROWING);
    ...
    std::thread t1([&] {
        ...
        Transaction txn1(1);
        bool res = lock_manager_->lock_shared_on_table(&txn1, tab_fd);
        operation.push_back(1);
        EXPECT_EQ(res, true);
        EXPECT_EQ(txn1.get_state(), TransactionState::GROWING);
