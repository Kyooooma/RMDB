...
bool LockManager::lock_shared_on_table(Transaction *txn, int tab_fd) {
    txn->set_state(TransactionState::GROWING);
    if(txn->lock_table_->count(tab_fd)){
        return true;
    }
    std::unique_lock<std::mutex> lock(latch_);
    //未持有锁
    if(!my_lock_table_.count(tab_fd) || my_lock_table_[tab_fd].cnt == 0){
        //该表没有锁, 直接申请
        my_lock_table_[tab_fd] = {LockMode::SHARED, 1};
        txn->lock_table_->insert(tab_fd);
        return true;
    }
    // 该表有锁
    auto &info = my_lock_table_[tab_fd];
    if(info.mode == LockMode::EXLUCSIVE){
        //该表有写锁，申请失败
        throw TransactionAbortException(txn->get_transaction_id(), AbortReason::DEADLOCK_PREVENTION);
    }
    //该表有读锁，申请成功
...
bool LockManager::lock_exclusive_on_table(Transaction *txn, int tab_fd) {
    ...
    if(!my_lock_table_.count(tab_fd) || my_lock_table_[tab_fd].cnt == 0){
        //该表没有锁, 直接申请
        my_lock_table_[tab_fd] = {LockMode::EXLUCSIVE, 1};
        txn->lock_table_->insert(tab_fd);
        return true;
    }
    // 该表有锁
...
bool LockManager::unlock(Transaction *txn) {
    ...
    for(auto i : *txn->lock_table_){
        my_lock_table_[i].cnt -= 1;
    }
    return true;
}
